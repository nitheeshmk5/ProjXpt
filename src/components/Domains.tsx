...
const domains = [
  {
    ...
    description: "Modern responsive websites with React, Angular, Vue.js, or full-stack solutions.",
    icon: "🌐",
  ...
  },
  {
    ...
    description: "Intelligent systems that learn patterns, predict outcomes, and make decisions.",
    icon: "🤖",
    color: "bg-genz-purple/10 text-genz-purple",
    tags: ["TensorFlow", "PyTorch", "NLP", "Computer Vision"]
  },
  {
    name: "IoT Projects",
    description: "Connect physical devices to the digital world for smart applications.",
    icon: "📱",
    color: "bg-genz-green/10 text-genz-green",
...
const Domains = () => {
  return (
    <div id="domains" className="py-20 px-6 md:px-10 bg-white/50 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {domains.map((domain, index) => (
            <div key={index} className="genz-card p-8 hover:translate-y-[-5px]">
              <div className={`text-4xl mb-4 inline-block p-3 rounded-xl ${domain.color}`}>{domain.icon}</div>
              ...
              <div className="flex flex-wrap gap-2">
                {domain.tags.map((tag, idx) => (
                  <Badge key={idx} variant="outline" className={`${domain.color} border-0`}>
                    {tag}
                  </Badge>
                ))}
              </div>
